...
// without pointers in the array stored not the Wall/Field/Pawn but its cut to GameObject that is about nothing
	...
	for (int i = 0; i < MAP_HEIGHT; i++)
	{
		for (int j = 0; j < MAP_WIDTH; j++)
		{
			...
			else if (gameData[i][j]->getObjectType() == wall)
			{
				...
				SetConsoleTextAttribute(hConsole, 144); // 67 112 144
